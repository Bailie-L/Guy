...
ACT_NAME = 'auto_skill_1757705565_46'
...
def act(ctx):
    ...
    if not q:
        return 0.0
    boosted = 0
    if 'mc_move_to_air' in q and q['mc_move_to_air'] > 0.0:
        q['mc_move_to_air'] = q['mc_move_to_air'] * 1.02
        boosted += 1
    if 'wolf_actions' in q and q['wolf_actions'] > 0.0:
        q['wolf_actions'] = q['wolf_actions'] * 1.02
        boosted += 1
    if 'wolf_unstuck' in q and q['wolf_unstuck'] > 0.0:
        q['wolf_unstuck'] = q['wolf_unstuck'] * 1.02
        boosted += 1
